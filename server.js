...
function normalizePrice(value) {
    ...
    if (
        ...
        if (
            commaParts.length > 1 &&
            commaParts[commaParts.length - 1].length <= 2
        ) {
...
async function searchUzum(query) {
    ...
    const data = await parseApiFetch(
        url.toString()
    );
    ...
    return items.map((item, index) => {
        ...
        const directProductUrl =
            findDirectProductUrl(item, "uzum");
...
app.get("/api/search", async (req, res) => {
    ...
    try {
        ...
        return res.status(502).json({
            ...
            message:
                "Product provider is temporarily unavailable. Please try again later."
        });
    }
